...
export default function ChangeEmailDialog({ user }) {
  ...
  return (
    <>
      <input type="checkbox" id="email-modal-check" className="modal-toggle" />
      <div className="modal">
            ...
            <>
              <div className="flex flex-col gap-2 p-2">
                <label className="text-stone-400">
                  {t("description.change_email")}
                </label>
                <label className="text-lg">{t("title.current_email")}</label>
                <label className="mb-2 text-stone-400 text-lg">
                  {user?.email}
                </label>
                <label className="text-lg">{t("title.new_email")}</label>
                <GradientBorderWrapper>
                  <input
                    type="email"
                    value={newEmail}
                    onChange={(e) => setNewEmail(e.target.value)}
                    placeholder={t('description.input_your_email')}
            ...
            <>
              <div className="flex flex-col gap-4 p-2 mb-4">
                <label className="text-stone-400 mb-4">
                  {t("description.already_sent_otp", { address: newEmail })}
                </label>
