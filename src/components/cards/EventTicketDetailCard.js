...
export default function EventTicketDetailCard({ ticket, onAction, eventType }) {
  ...
  return (
    <div className={`w-full ${ticket.sold?'opacity-60':'cursor-pointer '} `} onClick={onTap}>
      <GradientBorderWrapper>
        <div className="flex gap-2 justify-between p-4 items-center ">
          <div className="flex  gap-2">
            <div className="avatar h-12 w-12 flex-1 aspect-square">
                <LazyLoadImage src = {ticket.avatar} alt="" effect="blur" className="aspect-square  rounded-full "/>
            </div>
            <div className="flex flex-col">
                <h5>{eventType  }</h5>
                <h6 className="text-stone-500">{fDescriptionTypeDate(ticket.date)}</h6>
